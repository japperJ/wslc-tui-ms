...
var AllCommands = map[string][]commands.Command{
	"Container": {
		...
		{
			Name:        "cp",
			Full:        "wslc container cp {source} {target}",
			Category:    "Container",
			Description: "Copy a tar archive to or from a container",
			Usage:       "wslc container cp [flags] SRC_PATH DEST_PATH",
			Examples: []string{
				"wslc container cp ./backup.tar mycontainer:/tmp/backup.tar",
				"wslc container cp mycontainer:/tmp/backup.tar ./backup.tar",
			},
			Flags: []commands.Flag{
				{Short: "", Long: "--archive", Description: "Preserve archive metadata when copying"},
			},
			Difficulty: "intermediate",
			Tags:       []string{"cp", "copy", "upload", "download", "tar", "archive"},
		},
		{
			Name:        "prune",
	...
	"Session": {
		{
			...
			Flags: []commands.Flag{
				{Short: "", Long: "--verbose", Description: "Show detailed information about the listed sessions"},
			},
			Difficulty: "beginner",
...
func catalogSchema(category, name string) *commands.CommandSchema {
	...
	r := func(name string, required, repeatable bool, resourceType commands.ResourceType) commands.Argument {
		return commands.Argument{Name: name, Required: required, Repeatable: repeatable, ResourceType: resourceType, PickerEnabled: true}
	}
	...
	schemas := map[string]*commands.CommandSchema{
		...
		"Container/logs":    {Arguments: []commands.Argument{r("container", true, false, commands.ResourceTypeContainer)}, Options: []commands.Option{b("--follow"), t("--tail"), t("--since"), b("--timestamps"), t("--tail-n")}},
		...
		"Session/enter":     {Arguments: []commands.Argument{r("session", true, false, commands.ResourceTypeSession)}},
		"Session/run":       {Arguments: []commands.Argument{r("session", true, false, commands.ResourceTypeSession), a("command", true, true)}, Options: []commands.Option{t("--cpus"), t("--memory"), t("--storage")}},
		"Session/shell":     {Arguments: []commands.Argument{r("session", true, false, commands.ResourceTypeSession)}, Options: []commands.Option{t("--cpus"), t("--memory")}},
